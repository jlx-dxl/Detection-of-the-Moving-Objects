...
#------------------------------------------------------------读取视频-----------------------------------------------------
# path = r'D:\for cv\yolov5-master\yolov5-master\test.avi'
...
#-------------------------------------------------------设置角点检测及光流法的参数-------------------------------------------
feature_params = dict(maxCorners = 200, qualityLevel = 0.3, minDistance = 9, blockSize = 9)# ShiTomasi 角点检测参数
lk_params = dict( winSize  = (19,19),maxLevel = 3,criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03))# lucas kanade光流法参数
color = np.random.randint(0,255,(100,3))# 创建随机颜色

#-------------------------------------------------------获取第一帧的灰度图像及其角点-----------------------------------------
cap = cv2.VideoCapture(0) #这里必要要重新捕获视频，否则.read()不能正确获取第一帧
